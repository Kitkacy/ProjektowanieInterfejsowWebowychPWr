...
async function addBooksToFirestore() {
  console.log('Starting to add books to Firestore...');
  const booksCollection = db.collection('books');
  
  try {
    for (const book of books) {
      const result = await booksCollection.add(book);
      console.log(`Added book "${book.title}" with ID: ${result.id}`);
    }
    
    console.log('All books have been successfully added to Firestore!');
  } catch (error) {
...
addBooksToFirestore()
  .then(() => {
    console.log('Book import script completed.');
    process.exit(0);
  })
  .catch(error => {
